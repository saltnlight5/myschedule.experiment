...
/**
 * This unit test will explore different ways to schedule job in Quartz(2.x). We will not assert any values, but mostly
 * will start a scheduler to run for few seconds and observe the log output.
 * 
 * @author Zemian Deng
 */
public class ScheduleJobTest {
	...
	public void testScheduleOnetimeJobNow() throws Exception {
		...
		Trigger trigger = newTrigger().withIdentity("job1").withSchedule(simpleSchedule()).build();
	...
	public void testScheduleOnetimeJobWithStartTime() throws Exception {
		...
		JobDetail job = newJob(SimpleJob.class).withIdentity("job1").build();
		Trigger trigger = newTrigger().withIdentity("job1").withSchedule(simpleSchedule()).startAt(startTime).build();
	...
	public void testScheduleRepeatJob() throws Exception {
		...
		JobDetail job = newJob(SimpleJob.class).withIdentity("job1").build();
		Trigger trigger = newTrigger().withIdentity("job1").withSchedule(repeatSecondlyForTotalCount(repeatCount))
				.build();
	...
	public void testScheduleRepeatJob2() throws Exception {
		...
		JobDetail job = newJob(SimpleJob.class).withIdentity("job1").build();
		Trigger trigger = newTrigger().withIdentity("job1")
				.withSchedule(simpleSchedule().withRepeatCount(repeatCount).withIntervalInMilliseconds(repeatInterval))
				.build();
	...
	public void testScheduleRepeatJobForever() throws Exception {
		...
		JobDetail job = newJob(SimpleJob.class).withIdentity("job1").build();
		Trigger trigger = newTrigger().withIdentity("job1")
				.withSchedule(simpleSchedule().repeatForever().withIntervalInMilliseconds(repeatInterval)).build();
	...
	public void testScheduleCronJob() throws Exception {
		...
		JobDetail job = newJob(SimpleJob.class).withIdentity("job1").build();
		Trigger trigger = newTrigger().withIdentity("job1").withSchedule(cronSchedule(cronExpr)).build();
